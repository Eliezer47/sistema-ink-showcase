    ...
    <div className="lab-module lab-panel-module">
      ...
      <div className="lab-panel-grid">
        <section className="lab-order-card">
          ...
          <dl><div><dt>Cliente</dt><dd>{LAB_SCENARIO.customer.name}</dd></div><div><dt>Entrega prometida</dt><dd>{LAB_SCENARIO.deliveryDate}</dd></div><div><dt>Calidad</dt><dd>{snapshot.qualityStatus}</dd></div><div><dt>Entrega</dt><dd>{snapshot.deliveryStatus}</dd></div></dl>
...
function LabCash({ state, dispatch }: Pick<LabModuleSurfaceProps, "state" | "dispatch">) {
  ...
  return (
    <div className="lab-module">
      ...
      <div className="lab-cash-grid"><article><span>TOTAL</span><strong>{currency.format(LAB_SCENARIO.totalAmount)}</strong></article><article className="positive"><span>APLICADO</span><strong>{currency.format(snapshot.paidAmount)}</strong></article><article className={snapshot.balanceAmount ? "warning" : "positive"}><span>SALDO</span><strong>{currency.format(snapshot.balanceAmount)}</strong></article><article><span>FORMA SIMULADA</span><strong>{snapshot.paidAmount ? "Efectivo LAB" : "Sin movimientos"}</strong></article></div>
      <div className="lab-record-layout"><section className="lab-payment-card"><header><div><small>PEDIDO</small><strong>{LAB_SCENARIO.orderReference}</strong></div><span>{snapshot.balanceAmount ? "Saldo abierto" : "Liquidado"}</span></header><div className="lab-payment-breakdown"><div><span>Subtotal</span><strong>{currency.format(LAB_SCENARIO.totalAmount)}</strong></div><div><span>Anticipo esperado</span><strong>{currency.format(LAB_SCENARIO.depositAmount)}</strong></div><div><span>Pagos aplicados</span><strong>{currency.format(snapshot.paidAmount)}</strong></div><div><span>Saldo actual</span><strong>{currency.format(snapshot.balanceAmount)}</strong></div></div><p>No se genera recibo fiscal, transacción bancaria ni movimiento fuera de esta página.</p></section><LabActivity state={state} /></div>
    </div>
  );
}
...
function LabFinance({ state }: Pick<LabModuleSurfaceProps, "state">) {
  ...
  return (
    <div className="lab-module">
      ...
      <div className="lab-finance-grid"><article><span>VALOR DEL PEDIDO</span><strong>{currency.format(LAB_SCENARIO.totalAmount)}</strong><small>{LAB_SCENARIO.orderReference}</small></article><article className="positive"><span>RECUPERADO</span><strong>{currency.format(snapshot.paidAmount)}</strong><small>Movimientos LAB</small></article><article className={snapshot.balanceAmount ? "warning" : "positive"}><span>POR COBRAR</span><strong>{currency.format(snapshot.balanceAmount)}</strong><small>{snapshot.balanceAmount ? "Saldo ficticio" : "Ciclo liquidado"}</small></article><article><span>ESTADO OPERATIVO</span><strong>{snapshot.orderStatus}</strong><small>{snapshot.currentStage.label}</small></article></div>
      <div className="lab-record-layout"><section className="lab-record-list"><header><span>Documento</span><span>Cliente</span><span>Origen</span><span>Estado</span><span>Saldo</span></header><div className="is-selected"><strong>{LAB_SCENARIO.orderReference}</strong><span>{LAB_SCENARIO.customer.name}</span><span>Modo LAB</span><span>{snapshot.balanceAmount ? "Pendiente" : "Pagado"}</span><strong>{currency.format(snapshot.balanceAmount)}</strong></div></section><aside className="lab-side-card"><span>TRAZABILIDAD</span><h3>Sin contabilidad real</h3><p>Este resumen verifica visualmente cómo un cobro cambia el saldo del pedido. No crea asientos, impuestos ni documentos oficiales.</p></aside></div>
    </div>
  );
}
